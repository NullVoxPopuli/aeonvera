...
describe StripePaymentHandler do
  describe '#handle_stripe_charge' do
    ...
    it 'correctly assigns the fields' do
      allow(successful_charge).to receive(:paid) { successful_charge['paid'] }
      allow(successful_charge).to receive(:amount) { successful_charge['amount'] }
      ...
      expect(o.payment_received_at).to eq Time.at(1_439_853_046)
    end
  end
end
