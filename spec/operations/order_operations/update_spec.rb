...
describe OrderOperations::Update do
  ...
  context 'editing the contents of the order' do
    ...
    context 'adds an order line item' do
      let(:competition2){ create(:competition, event: event, kind: Competition::SOLO_JAZZ) }
          ...
          "data"=>{
            ...
            "relationships"=>{
              ...
              "order-line-items"=>{"data"=>[
                ...
                { # no id, cause this item is new
                  "attributes"=>{"quantity"=>1, "price"=>competition2.current_price, "partner-name"=>nil, "dance-orientation"=>nil, "size"=>nil, "payment-token"=>nil},
                  "relationships"=>{"line-item"=>{"data"=>{"type"=>"line-items", "id"=>competition2.id}},
                  "order"=>{"data"=>{"type"=>"orders", "id"=>order.id}}}, "type"=>"order-line-items"},
                {"id"=>item2.id,
      ...
      before(:each) do
        # reload to the order, so that it gets the order line items
        order.reload
    ...
    # I don't need to test 'remove' because remove happens via DELETE
    # request to the order_line_items controller
    context 'removes an order line item' do
    end
    ...
    context 'updates an existing line item' do
      ...
      it 'updates an order line item' do
        ...
        expect(new_sub_total).to eq package.current_price + old_total
      end

      it 'does not update if an order line item becomes invalid' do
        @params_for_action[:order_line_items_attributes][0][:quantity] = nil
        operation = klass.new(user, params, @params_for_action)
        ...
        Delorean.time_travel_to("1 month from now") do
          ...
          # have to call to_date to get rid of the arbitrary
          # not stored in the database nano seconds....
          expect(updated_at.to_date).to eq model.updated_at.to_date
        end
      end
    end
  end
